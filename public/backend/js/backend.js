$(function () {
      ...
      success: function (res) {
        ...
        $.each(res, function(i, item) {
          if(item.item_name != null){
            html += '<tr> \
            <td>'+item.item_name+' ('+item.item_product_id+')</td>\
  ...
  $(document).on("click", ".export_order_data", function () {
    ...
    if(order_start_date == ''){return alert('Select Start date!');}
    if(order_end_date == ''){ return alert('Select End date!');}
        ...
        if(res.status == true) {
          if(res.getAllOrders) {
            ...
            //  let date_purchased = formatYmd(new Date(item.date_purchased));
                html += '<p class="export_order_data_detail_X" data-order-number="'+item.order_number+'">'+item.billing_email+'</p>';
            });
